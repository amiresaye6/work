...
async function processMatchedProducts(inputFileName) {
  ...
  for (const match of matchedProducts) {
    const nahdiProduct = match.nahdiProduct;
    if (!nahdiProduct || !nahdiProduct.productId || !nahdiProduct.url) {
        console.warn('Skipping a malformed match object:', match);
        continue;
    }
    ...
    const combinedData = {
      productId: productId,
      pharmacistaId: match.pharmacistaProduct['Internal Reference'],
      url_en: enUrl,
      url_ar: arUrl,
      ...
      images: enData.data.images.length > 0 ? enData.data.images : arData.data.images,
...
(async () => {
  ...
  await processMatchedProducts("matched_products.json");
})();